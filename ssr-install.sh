...
#安装chacha20的依赖库
wget -N --no-check-certificate https://raw.githubusercontent.com/mrwh85/shadowsocks/master/libsodium-1.0.12.tar.gz
...
#git安装ssr
git clone https://github.com/mrwh85/shadowsocksr-1.git ${ssrdir}
cd ${ssrdir}
...
if [ "$OS" == 'CentOS' ]; then
	if ! wget --no-check-certificate https://raw.githubusercontent.com/mrwh85/shadowsocks/master/ssr -O /etc/init.d/ssr; then
		echo "Failed to download ssr chkconfig file!"
		exit 1
	fi
else
	if ! wget --no-check-certificate https://raw.githubusercontent.com/mrwh85/shadowsocks/master/ssr-debian -O /etc/init.d/ssr; then
		echo "Failed to download ssr chkconfig file!"
		exit 1
	fi
fi
...
#下载定制脚本到目录
if ! wget --no-check-certificate https://raw.githubusercontent.com/mrwh85/shadowsocks/master/ssr.sh -O ${ssrdir}ssr.sh; then
